...
function Item({ rows, onView, onEdit, onDelete, total, pageSize, pageSizeOptions, currentPage, totalPages, safeCurrentPage, onPageSizeChange, onJumpToPage, onPrevPage, onNextPage, sortKey, sortDir, onSort }) {
  const columns = [
    { key: "name", label: "Item", width: 180 },
    { key: "sku", label: "Item Code", width: 140 },
      ...
      <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
                ...
                <tbody>
                  {(rows || []).map((row) => (
                    <tr key={row.id} onClick={() => onView?.(row)} tabIndex={onView ? 0 : -1} style={{ cursor: onView ? "pointer" : "default" }}>
                      {columns.map((col) => (
                        <td key={col.key} style={{ ...alignStyle, width: col.width }}>{col.key === "name" ? toTitleCase(row[col.key] || "—") : (col.key === "price" ? `${getCurrencySymbol()}${row[col.key] || 0}` : row[col.key] || "—")}</td>
                      ))}
        ...
        <div className="pagination-controls data-table-pagination" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0 0", marginTop: 8, borderTop: "1px solid var(--color-border)", flexWrap: "wrap", gap: 12, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>Total Item {total}</span>
          </div>
