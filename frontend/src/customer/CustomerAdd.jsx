...
function CustomerAdd({ complete, salesPersonOptions = [] }) {
  const [form, setForm] = useState({
    name: "", phone: "", alternatePhone: "",
    houseNo: "", locality: "", city: "", state: "", pincode: "", address: "", salesPerson: ""
  });
  ...
  const save = async (e) => {
    ...
    const r = await request("/customers", { method: "POST", body: JSON.stringify(body) });
    setSaving(false);
    ...
    <form className="form card" onSubmit={save}>
      ...
      <label>Alternate Phone
        <input type="tel" inputMode="numeric" pattern="[0-9]{10}" title="Enter exactly 10 digits." maxLength={10} value={form.alternatePhone}
          onChange={(e) => setForm({ ...form, alternatePhone: e.target.value.replace(/[^0-9]/g, "") })}
          placeholder="Optional 10-digit number" style={inputStyle} />
      </label>
